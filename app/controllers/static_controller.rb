...
class StaticController < ApplicationController
  ...
  def campaign_manager_landing
    ...
    @survey_evaluations = Response.find_by_sql(" 
        ...
        year = DateTime.now.year if params[:year].blank? # setting current year to default .. set this to pick the latest year  later (like ews).
  ...
  def load_chart
    ...
    logger.debug "load_chart - Year Selected is :#{params[:year]}"
    logger.debug "load_chart - Chart Type Selected is :#{params[:chart_type]}"
    @stacked = false
    ...
    year = DateTime.now.year if params[:year].blank? # setting current year to default .. set this to pick the latest year  later (like ews).
    logger.debug "load_chart - params[:year] in load_chart is #{params[:year]}, whereas year is #{year}"
      # Analytics Tab : 
      _cmp = Campaign.new
      ...
      when 2
            ...
            logger.debug "load_chart - specialists is :::: #{specialists.inspect}"
            ...
            specialists.each_with_index do |sp, index|
              logger.debug "load_chart - sp[#{index}] ::: is ::: #{sp.inspect}"
            ...
            specialist_arr.each do |spec|
              ...
              logger.debug "load_chart - series_completed is #{series_completed}"
              
              data["Draft"].each do |draft|
                #logger.debug "draft is #{draft} "
                series_draft = series_draft.push(draft[1]) if draft[0] == spec
              end
              logger.debug "load_chart - series_draft is #{series_draft}"
              ...
              logger.debug "load_chart - series_inprocess is #{series_inprocess}"
              
            end
            logger.debug "load_chart - -->>> specialist_arr is : #{specialist_arr}"
      ...
      when 3
            ...
            specialist_arr.each do |spec|
              ...
              logger.debug "load_chart - series_T1 is #{series_T1}"
              
              data["T2"].each do |t2|
                series_T2 = series_T2.push(t2[1]) if t2[0] == spec
              end
              logger.debug "load_chart - series_T2 is #{series_T2}"
              
             data["T3"].each do |t3|
                series_T3 = series_T3.push(t3[1]) if t3[0] == spec
              end
              logger.debug "load_chart - series_T3 is #{series_T3}"
              
              data["T4"].each do |t4|
                series_T4 = series_T4.push(t4[1]) if t4[0] == spec
              end
              logger.debug "load_chart - series_T4 is #{series_T4}"
              
              data["T5"].each do |t5|
                series_T5 = series_T5.push(t5[1]) if t5[0] == spec
              end
              logger.debug "load_chart - series_T5 is #{series_T5}"
              
            end
            logger.debug "load_chart - -->>> specialist_arr is : #{specialist_arr}"
            logger.debug "load_chart - -->>> data after balancing is : #{data}"
            @chart = LazyHighCharts::HighChart.new('graph') do |f|
